...
function calculateNumberOfMatch(tryNumber, userLottoNumber, winningNumber) {
...
function getRank(userLottoNumberArray, userCorrecNumber, bonusNumberRead) {
  if (userCorrecNumber < LOTTO_INFORMATION.Min_Match) return null;

  if (userCorrecNumber === LOTTO_INFORMATION.MATCH_COUNTER.THREE)
    return LOTTO_INFORMATION.LOTTO_RANKING_INDEX.FIFTH; // 5등
  if (userCorrecNumber === LOTTO_INFORMATION.MATCH_COUNTER.FOUR)
    return LOTTO_INFORMATION.LOTTO_RANKING_INDEX.FOURTH; // 4등
  if (userCorrecNumber === LOTTO_INFORMATION.MATCH_COUNTER.SIX)
    return LOTTO_INFORMATION.LOTTO_RANKING_INDEX.FIRST; // 1등
  if (userCorrecNumber === LOTTO_INFORMATION.MATCH_COUNTER.FIVE) {
    const hasBounus = userLottoNumberArray.includes(bonusNumberRead);
    if (hasBounus) {
      return LOTTO_INFORMATION.LOTTO_RANKING_INDEX.SECOND; //2등
    }
    return LOTTO_INFORMATION.LOTTO_RANKING_INDEX.THIRD; //3등
  }
  return null;
}
...
export default calculateNumberOfMatch;
